...
type NetworkPolicyHandler struct{ service services.NetworkPolicyService }

func NewNetworkPolicyHandler(s services.NetworkPolicyService) *NetworkPolicyHandler {
	return &NetworkPolicyHandler{service: s}
}
...
func (h *NetworkPolicyHandler) Get(c *gin.Context) {
...
func (h *NetworkPolicyHandler) Create(c *gin.Context) {
	...
	err := h.service.Create(c.Request.Context(), &np)
	if err != nil {
		c.JSON(http.StatusInternalServerError, responses.Error(err.Error()))
		return
	}

	c.JSON(http.StatusCreated, responses.Success(np))
}

func (h *NetworkPolicyHandler) Update(c *gin.Context) {
	var np networkingv1.NetworkPolicy
	...
	err := h.service.Update(c.Request.Context(), &np)
	if err != nil {
		if errors.Is(err, services.ErrNotFound) {
			c.JSON(http.StatusNotFound, responses.Error(err.Error()))
			return
	...
	c.JSON(http.StatusOK, responses.Success(np))
}
